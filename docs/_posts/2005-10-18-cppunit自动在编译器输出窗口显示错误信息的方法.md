...
另外，对于在外面执行的情况，可以在Project Settings的Debug中设置执行目录（因为我希望测试数据在另外一个目录，而程序在这个目录下执行）。而使用Post-build的方法不能指定执行的当前目录（谁知道？），现在我的方法是在main()中直接用 `SetCurrentDirectory()` 进行设置。
